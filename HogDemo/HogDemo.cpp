...
{
	string test_img = "D:\\image\\h2.jpg";
	if(argc > 1){
		test_img = argv[1];
	}
	...
	if(TRAIN)
	{
		...
		ifstream finPos("D:/image/INRIAPerson/train_64x128_H96/pos.txt");//正样本图片的文件名列表
		//ifstream finPos("PersonFromVOC2012List.txt");//正样本图片的文件名列表
		ifstream finNeg("D:/image/INRIAPerson/train_64x128_H96/nn.txt");//负样本图片的文件名列表
		...
		for(int num=0; num<PosSamNO && getline(finPos,ImgName); num++)
		{
			...
			ImgName = "D:\\image\\INRIAPerson\\train_64x128_H96\\pos\\" + ImgName;//加上正样本的路径名
		...
		{
			cout<<"处理："<<ImgName<<endl;
			ImgName = "D:\\image\\INRIAPerson\\train_64x128_H96\\nn\\" + ImgName;//加上负样本的路径名
			Mat src = imread(ImgName);//读取图片
	...
	{
		//svm.load("SVM_HOG_2400PosINRIA_12000Neg(负样本增多误报减少).xml");//从XML文件读取训练好的SVM模型
